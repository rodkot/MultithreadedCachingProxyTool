...
int ServerScheduler::connect_to_server(Server *server) {
    ...
    char *host = (char *) calloc(server->request->host_len + 1, sizeof(char));
    strncpy(host, server->request->host, server->request->host_len);
    host[server->request->host_len] = '\0';
    ...
    int errcode = getaddrinfo(host, "80", &hints, &result);
    if (errcode != 0) {
        free(host);
        perror("getaddrinfo");
